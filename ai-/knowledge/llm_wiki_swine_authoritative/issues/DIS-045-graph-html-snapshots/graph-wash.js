...
(function () {
  ...
  function setDrawerActions(mode, node) {
    ...
    if (queueAction) {
      queueAction.textContent = "查看变更日志";
      queueAction.disabled = false;
    }
    if (sourceAction) {
      sourceAction.textContent = "打开变化页";
  ...
  function renderChangeList(items, emptyText, limit) {
    const list = Array.isArray(items) ? items.slice(0, limit || 8) : [];
    if (!list.length) return `<p>${escapeHtml(emptyText)}</p>`;
    return `<ul>${list.map((item) => `<li><code>${escapeHtml(item)}</code></li>`).join("")}</ul>`;
  }

  function renderCrudCounts(crudCounts) {
    if (!crudCounts || typeof crudCounts !== "object") {
      return "<p>暂无 CRUD 统计。</p>";
    }
    const rows = [];
    Object.keys(crudCounts).forEach((action) => {
      const groups = crudCounts[action] || {};
      Object.keys(groups).forEach((group) => {
        rows.push(`${action}: ${group} ${groups[group]}`);
      });
    });
    if (!rows.length) return "<p>暂无 CRUD 统计。</p>";
    return `<ul>${rows.map((row) => `<li>${escapeHtml(row)}</li>`).join("")}</ul>`;
  }

  function renderGraphChangeLog() {
    const changeLog = DATA && DATA.change_log ? DATA.change_log : {};
    const summary = changeLog.summary || {};
    const evidence = changeLog.evidence_files || {};
    if (app) {
      app.dataset.reading = "1";
      app.dataset.startPreview = "0";
    }
    if (drawer) drawer.dataset.state = "change-log";
    setDrawerActions("change-log", null);

    const kind = document.getElementById("drawer-kind");
    const title = document.getElementById("drawer-title");
    const subtitle = document.getElementById("drawer-subtitle");
    const summaryBox = document.getElementById("drawer-summary");
    const neighborCount = document.getElementById("drawer-neighbor-count");
    const content = document.getElementById("drawer-content");

    if (kind) kind.innerHTML = `<span class="spark"></span>GRAPH · UPDATE LOG`;
    if (title) title.textContent = changeLog.title || "图谱更新日志";
    if (subtitle) subtitle.textContent = `${changeLog.generated_at || ""} · ${changeLog.subtitle || "更新前后对比"}`;
    if (summaryBox) {
      summaryBox.textContent = `节点 ${summary.nodes_before || 0} -> ${summary.nodes_after || 0} (${formatSignedNumber(summary.delta_nodes)}), 关系 ${summary.links_before || 0} -> ${summary.links_after || 0} (${formatSignedNumber(summary.delta_links)})。`;
    }
    if (neighborCount) neighborCount.textContent = `${summary.added_nodes || 0} 节点 / ${summary.added_links || 0} 边`;

    if (content) {
      content.innerHTML = `
        <h3>更新前后对比</h3>
        <ul>
          <li>节点：${escapeHtml(summary.nodes_before || 0)} -> ${escapeHtml(summary.nodes_after || 0)}，变化 ${escapeHtml(formatSignedNumber(summary.delta_nodes))}</li>
          <li>关系：${escapeHtml(summary.links_before || 0)} -> ${escapeHtml(summary.links_after || 0)}，变化 ${escapeHtml(formatSignedNumber(summary.delta_links))}</li>
          <li>事实入图：${escapeHtml(summary.facts_in_graph || "未记录")}</li>
          <li>新增/删除/修改节点：${escapeHtml(summary.added_nodes || 0)} / ${escapeHtml(summary.removed_nodes || 0)} / ${escapeHtml(summary.changed_nodes || 0)}</li>
          <li>新增/删除/修改关系：${escapeHtml(summary.added_links || 0)} / ${escapeHtml(summary.removed_links || 0)} / ${escapeHtml(summary.changed_links || 0)}</li>
        </ul>
        <h3>CRUD 统计</h3>
        ${renderCrudCounts(changeLog.crud_counts)}
        <h3>新增节点示例</h3>
        ${renderChangeList(changeLog.added_nodes, "本轮没有新增节点。", 8)}
        <h3>新增关系示例</h3>
        ${renderChangeList(changeLog.added_links, "本轮没有新增关系。", 6)}
        <h3>合理性说明</h3>
        <p>${escapeHtml(changeLog.reasonableness || "请结合 graph diff 和变更记录确认变化原因。")}</p>
        <h3>证据文件</h3>
        <ul>
          <li><code>${escapeHtml(evidence.diff_json || "issues/graph_change_diff_last.json")}</code></li>
          <li><code>${escapeHtml(evidence.diff_md || "issues/graph_change_diff_last.md")}</code></li>
          <li><code>${escapeHtml(evidence.crud_log || "issues/wiki_crud_change_log.jsonl")}</code></li>
          <li><code>${escapeHtml(evidence.changes_html || "wiki/knowledge-graph-changes.html")}</code></li>
        </ul>
      `;
    }
    ...
    if (neighborList) {
      ...
      note.textContent = "右侧面板固定展示图谱更新日志；点击节点只影响中间图谱高亮，便于对照变化来源。";
      neighborList.appendChild(note);
    }
  }
  ...
  function setupControls() {
    ...
    const queueAction = document.getElementById("queue-action");
    if (queueAction) queueAction.addEventListener("click", renderGraphChangeLog);

    const sourceAction = document.getElementById("source-action");
    if (sourceAction) {
      sourceAction.addEventListener("click", () => {
        window.location.href = "knowledge-graph-changes.html";
      });
    }
  }
