...
test_oriental_atlas_has_required_copy() {
    ...
    assert_file_contains "$html" "东方编辑部 × 数字山水风交互式知识图谱"
    ...
    assert_file_contains "$html" "东方编辑部 × 数字山水风交互式知识图谱"
    assert_file_contains "$html" "全部离线 · 不依赖服务器"
    assert_file_contains "$html" "双击 HTML 即可离线阅读"
    assert_file_contains "$html" "<h3>正文</h3>"
