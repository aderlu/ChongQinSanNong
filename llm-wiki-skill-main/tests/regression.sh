...
make_path_without_python() {
    ...
    for cmd in bash dirname pwd mkdir tr awk sed sort cut head tail find wc uname date printf grep cat rm cp chmod mktemp; do
        cmd_path="$(PATH="/usr/bin:/bin:/usr/sbin:/sbin" type -P "$cmd" 2>/dev/null || true)"
        if [ -n "$cmd_path" ]; then
...
test_install_registers_and_uninstalls_session_start_hook() {
    local tmp_dir settings_path output jq_path
    tmp_dir="$(mktemp -d)"
    trap 'rm -rf "$tmp_dir"' RETURN
...
exit 0'
    ...
    [ -n "$jq_path" ] || fail "jq should be available for hook registration test"
    printf '#!/bin/sh\nexec "%s" "$@"\n' "$jq_path" > "$tmp_dir/bin/jq"
