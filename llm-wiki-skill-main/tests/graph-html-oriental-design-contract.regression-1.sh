...
test_oriental_design_contract_hooks() {
    ...
    assert_file_contains "$html" 'class="feature-grid"'
    assert_file_contains "$html" "数字山水知识图谱"
    assert_file_contains "$html" "图谱阅读体验打磨"
    assert_file_contains "$html" "本地阅读动线"
    assert_file_contains "$html" "零配置初始化"
    assert_file_contains "$html" "结构化 Wiki"
    assert_file_contains "$html" "置信度标注"
    assert_file_contains "$html" "智能缓存"
    assert_file_contains "$html" "对话结晶化"
    assert_file_contains "$html" "自动上下文注入"
    assert_file_contains "$html" "多格式分析"
